...
export const fetchApi = createAsyncThunk("fetchApi", async () => {
    const response = await axios.get("http://localhost:5100/item")
    return response.data
})

// Increment quantity
export const addQuantity = createAsyncThunk("addQuantity", async (id) => {
    const response = await axios.get(`http://localhost:5100/item/${id}`)
    const updatedItem = { 
        ...response.data, 
        quantity: response.data.quantity + 1 
    }
    await axios.put(`http://localhost:5100/item/${id}`, updatedItem)
    return updatedItem
})
